...
<template>
    <div class="flex flex-col justify-center items-center w-full gap-4">
        <Card v-for="(result, index) in results" :key="result.champion.id"
            class="w-full bg-white/5 backdrop-blur-md border border-foreground/10 shadow-sm">
            ...
            <CardHeader class="flex flex-col justify-center items-center gap-4 mt-10 md:mt-0">
                <CardTitle class="text-3xl font-bold md:text-6xl">{{ result.champion.name }}</CardTitle>
                <CardDescription>
            ...
            <CardContent class="flex flex-col justify-center items-center w-full gap-4">
                <div v-if="result.items" class="flex flex-col items-center w-full gap-4">
                ...
                <div v-if="result.summonerSpells" class="flex flex-col justify-center items-center w-full gap-4">
                    ...
                    <div class="flex flex-row justify-center items-center w-full gap-4">
                        <div v-for="summonerSpell in result.summonerSpells" :key="summonerSpell.id"
                            class="flex flex-row flex-wrap justify-center items-center gap-4 w-1/4">
